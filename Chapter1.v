...
Proof. reflexivity.  Qed.
Example test_mult2: (mult 100 0) = 0.
Proof. reflexivity.  Qed.
Example test_mult3: (mult 102 2) = 204.
Proof. reflexivity.  Qed.
...
Fixpoint blt_nat_recursive (n m : nat) : bool :=
  match n, m with
    | O, O     => false
    | O, _     => true
    | _, O     => false
    | S n, S m => blt_nat_recursive n m
  end.
...
Definition blt_nat (n m : nat) : bool :=
  match n, m with
    | O, O => false
    | O, _ => true
    | S n', _ => andb (negb (beq_nat n m)) (beq_nat O (minus n m))
  end.
